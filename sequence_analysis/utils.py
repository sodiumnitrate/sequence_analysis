...
def kcal_mol_to_kbt(energy_in_kcal_mol,temperature):
    ...
    return energy_in_kcal_mol * (4184/6.0221409e23)/(1.38064852e-23*temperature)
...
def find_spans(full_string, matches):
    ...
    chars_absent_from_full_string = list(set(string.ascii_uppercase) - set(full_string))
    ...
    for i in range(n_points):
        ...
        for k in range(max_iter):
            ...
            if not clash:
                ...
                break
        if not placed:
            print(f"ERROR: node not placed in max_iter={max_iter}. Increase max_iter or decrease min_dist.")
            return None
