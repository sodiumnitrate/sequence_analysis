import os
from sequence_analysis.seq_set import seq_set
from sequence_analysis.sequence import sequence


class TestIO:
    def test_read_fasta(self):
        prot_test = seq_set(file_name="aux_files/test.fasta")
        assert (isinstance(prot_test, seq_set))
        assert (isinstance(prot_test.records, list))
        assert (len(prot_test.records) == 3)
        assert (prot_test.type == 'protein')

    def test_add_sequence(self):
        prot_test = seq_set()
        new_sequence = sequence("ALKALI", name="test_add")
        prot_test.add_sequence(new_sequence)
        assert (new_sequence.type == 'protein')
        assert (prot_test.type == 'protein')
        assert (len(prot_test.records) == 1)

    def test_add_sequence_by_list(self):
        prot_test = seq_set()
        new_sequence = sequence("ALKALI", name="test_add")
        new_sequence2 = sequence("ALKALIMDM", name="test_add2")
        prot_test.add_sequence([new_sequence, new_sequence2])
        assert (new_sequence.type == "protein")
        assert (new_sequence2.type == "protein")
        assert (len(prot_test) == 2)

    def test_read_dna_fasta(self):
        dna_test = seq_set(file_name="aux_files/dna_ex.fasta")
        assert (dna_test.type == 'dna')
        assert (len(dna_test.records) == 2)

    def test_write_fasta(self):
        prot_test = seq_set(file_name="aux_files/test.fasta")
        prot_test.write_fasta("aux_files/write_test.fasta")
        test_read = seq_set(file_name="aux_files/write_test.fasta")

        # make sure the two files are the same
        assert (len(prot_test) == len(test_read))

        for i in range(len(prot_test)):
            s1 = prot_test.records[i].seq
            s2 = test_read.records[i].seq
            assert (s1 == s2)

    def test_write_fasta_in_parts(self):
        test = seq_set(file_name="aux_files/test_2.fasta")
        test.write_fasta_in_parts("aux_files/write_parts.fasta", n_seq=2)
        files = [i for i in os.listdir("aux_files/") if i.startswith("write_parts")]
        
        assert len(files) == 6

    def test_write_fasta_seq(self):
        a = sequence("A"*100)
        a.name = "seq_a"
        b = sequence("G"*100)
        b.name = "seq_b"

        a.write_fasta("aux_files/write_fasta_seq.fa", append=False)
        b.write_fasta("aux_files/write_fasta_seq.fa")

        seqs = seq_set(file_name="aux_files/write_fasta_seq.fa")
        assert len(seqs) == 2
        assert seqs[0].name == 'seq_a'

    def test_read_fasta_filter(self):
        names = ['seq2', 'seq10']
        seqs = seq_set(file_name="aux_files/test_2.fasta", filter_by_names=names)

        assert len(seqs) == 2
        assert seqs[0].name == 'seq2'
        assert seqs[1].name == 'seq10'

    def test_read_fasta_filter_2(self):
        names = ['seq2', 'seq103']
        seqs = seq_set(file_name="aux_files/test_2.fasta", filter_by_names=names)
        assert len(seqs) == 1